...
template <typename T> struct HasteLayerNormGruOp : public OpKernel {
  explicit HasteLayerNormGruOp(OpKernelConstruction *context)
      : OpKernel(context) {
  ...
  void Compute(OpKernelContext *context) override {
    ...
    OP_REQUIRES_OK(context,
                   context->allocate_output(1, {memory_layout.NumElements()},
                                            &output_cache));
    ...
    const TensorShape tmp_Wx_norm_shape = {time_steps, batch_size,
                                           hidden_size * 3};
    Tensor tmp_Wx_norm;
    OP_REQUIRES_OK(context, context->allocate_temp(data_type, tmp_Wx_norm_shape,
                                                   &tmp_Wx_norm));

    const TensorShape tmp_Rh_norm_shape = {batch_size, hidden_size * 3};
    Tensor tmp_Rh_norm;
    OP_REQUIRES_OK(context, context->allocate_temp(data_type, tmp_Rh_norm_shape,
                                                   &tmp_Rh_norm));

    cudaMemset(output->flat<T>().data(), 0, output->AllocatedBytes());

    layer_norm::ForwardPass<T> layer_norm1(
        time_steps * batch_size, hidden_size * 3,
    ...
    layer_norm_gru::ForwardPass<T> gru(training_, batch_size, input_size,
                                       hidden_size, GetCublasHandle(context));

    gru.Run(time_steps, kernel.flat<T>().data(),
            recurrent_kernel.flat<T>().data(), bias.flat<T>().data(),
            recurrent_bias.flat<T>().data(), input.flat<T>().data(),
            output->flat<T>().data(), cache.data(), act_Wx.data(), layer_norm1,
            tmp_Wx_norm.flat<T>().data(), act_Rh.data(), layer_norm2,
            tmp_Rh_norm.flat<T>().data(), has_zoneout ? zoneout_prob_ : 0.0f,
...
template <typename T> struct HasteLayerNormGruGradOp : public OpKernel {
  explicit HasteLayerNormGruGradOp(OpKernelConstruction *context)
      : OpKernel(context) {}
  ...
  void Compute(OpKernelContext *context) override {
    ...
    OP_REQUIRES_OK(context,
                   context->allocate_output(5, gamma.shape(), &dgamma));
    ...
    layer_norm_gru::BackwardPass<T> gru(batch_size, input_size, hidden_size,
                                        GetCublasHandle(context));

    gru.Run(time_steps, kernel.flat<T>().data(),
            recurrent_kernel.flat<T>().data(), bias.flat<T>().data(),
            recurrent_bias.flat<T>().data(), input.flat<T>().data(),
            h_vector.flat<T>().data(), cache.data(), dh_new.flat<T>().data(),
            dx->flat<T>().data(), dW->flat<T>().data(), dR->flat<T>().data(),
            dbx->flat<T>().data(), dbr->flat<T>().data(), dh.flat<T>().data(),
            dp.flat<T>().data(), dq.flat<T>().data(), layer_norm1, layer_norm2,
            has_zoneout ? zoneout_mask.flat<T>().data() : nullptr);
  }
};
