...
template <typename T> struct HasteLstmOp : public OpKernel {
  ...
  void Compute(OpKernelContext *context) override {
    ...
    if (training_) {
      OP_REQUIRES_OK(context,
                     context->allocate_output(2, activations_shape, &output_v));
    } else {
      // Return an empty tensor in inference mode and provide temp memory
      // to the forward pass instead.
      OP_REQUIRES_OK(context,
                     context->allocate_output(2, TensorShape({0}), &output_v));
      OP_REQUIRES_OK(
          context,
          context->allocate_temp(data_type, activations_shape, &output_v_temp));
    ...
    ForwardPass<T> forward =
        ForwardPass<T>(training_, batch_size, input_size, hidden_size,
    ...
    forward.Run(time_steps, kernel.flat<T>().data(),
                ...
                output_cell_state->flat<T>().data(), output_v->flat<T>().data(),
                tmp_Rh.flat<T>().data(), has_zoneout ? zoneout_prob_ : 0.0f,
...
template <typename T> struct HasteLstmGradOp : public OpKernel {
  ...
  void Compute(OpKernelContext *context) override {
    ...
    backward.Run(time_steps, kernel.flat<T>().data(),
                 ...
                 db->flat<T>().data(), dh.flat<T>().data(), dc.flat<T>().data(),
