...
template <typename T> struct HasteIndrnnOp : public OpKernel {
  ...
  void Compute(OpKernelContext *context) override {
    ...
    forward.Run(time_steps, kernel.flat<T>().data(),
                ...
                workspace.flat<T>().data(), has_zoneout ? zoneout_prob_ : 0.0f,
...
template <typename T> struct HasteIndrnnGradOp : public OpKernel {
  explicit HasteIndrnnGradOp(OpKernelConstruction *context)
      : OpKernel(context) {}
