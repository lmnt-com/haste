...
template <typename T> struct HasteLayerNormIndrnnOp : public OpKernel {
  explicit HasteLayerNormIndrnnOp(OpKernelConstruction *context)
      : OpKernel(context) {
  ...
  void Compute(OpKernelContext *context) override {
    ...
    OP_REQUIRES_OK(context, context->allocate_temp(data_type, workspace_shape,
                                                   &workspace));
    ...
    layer_norm_indrnn::ForwardPass<T> forward(training_, batch_size, input_size,
                                              hidden_size,
                                              GetCublasHandle(context));

    forward.Run(time_steps, kernel.flat<T>().data(),
                recurrent_scale.flat<T>().data(), bias.flat<T>().data(),
                input.flat<T>().data(), output->flat<T>().data(),
                workspace.flat<T>().data(), act_Wx.data(), layer_norm1,
...
template <typename T> struct HasteLayerNormIndrnnGradOp : public OpKernel {
  explicit HasteLayerNormIndrnnGradOp(OpKernelConstruction *context)
      : OpKernel(context) {}
  ...
  void Compute(OpKernelContext *context) override {
    ...
    layer_norm_indrnn::BackwardPass<T> backward(
        batch_size, input_size, hidden_size, GetCublasHandle(context));

    backward.Run(
        time_steps, kernel.flat<T>().data(), recurrent_scale.flat<T>().data(),
        bias.flat<T>().data(), input.flat<T>().data(),
        h_vector.flat<T>().data(), dh_new.flat<T>().data(),
        dx->flat<T>().data(), dW->flat<T>().data(), du->flat<T>().data(),
        db->flat<T>().data(), dh.flat<T>().data(), workspace.flat<T>().data(),
        layer_norm1, has_zoneout ? zoneout_mask.flat<T>().data() : nullptr);
  }
};
