...
std::vector<Tensor>
layer_norm_lstm_forward(bool training, float zoneout_prob, Tensor x, Tensor h0,
                        Tensor c0, Tensor kernel, Tensor recurrent_kernel,
                        Tensor bias, Tensor gamma, Tensor gamma_h,
                        Tensor beta_h, Tensor zoneout_mask) {
  ...
  Tensor output =
      torch::zeros({time_steps + 1, batch_size, hidden_size}, options);
  Tensor output_state =
      torch::zeros({time_steps + 1, batch_size, hidden_size}, options);
  Tensor act_Wx =
  ...
  Tensor act_c_norm =
      torch::empty({time_steps, batch_size, hidden_size}, options);
  ...
  AT_DISPATCH_FLOATING_TYPES(
      x.scalar_type(), "layer_norm_lstm_forward", ([&] {
        auto gamma_a = gamma.packed_accessor32<scalar_t, (int)2>();

        layer_norm::ForwardPass<scalar_t> layer_norm1(
            time_steps * batch_size, hidden_size * 4, gamma_a[0].data(),
            nullptr, act_Wx_norm_cache.data_ptr<scalar_t>());

        layer_norm::ForwardPass<scalar_t> layer_norm2(
            time_steps * batch_size, hidden_size * 4, gamma_a[1].data(),
            nullptr, act_Rh_norm_cache.data_ptr<scalar_t>());

        layer_norm::ForwardPass<scalar_t> layer_norm3(
            time_steps * batch_size, hidden_size, gamma_h.data_ptr<scalar_t>(),
            beta_h.data_ptr<scalar_t>(), act_c_norm_cache.data_ptr<scalar_t>());

        layer_norm_lstm::ForwardPass<scalar_t> lstm(
            training, batch_size, input_size, hidden_size,
            at::cuda::getCurrentCUDABlasHandle(),
            at::cuda::getCurrentCUDAStream());

        lstm.Run(time_steps, kernel.data_ptr<scalar_t>(),
                 recurrent_kernel.data_ptr<scalar_t>(),
                 bias.data_ptr<scalar_t>(), x.data_ptr<scalar_t>(),
                 output.data_ptr<scalar_t>(), output_state.data_ptr<scalar_t>(),
                 act_Wx.data_ptr<scalar_t>(), tmp_Rh.data_ptr<scalar_t>(),
                 layer_norm1, act_Wx_norm.data_ptr<scalar_t>(),
                 act_Rh.data_ptr<scalar_t>(), layer_norm2, layer_norm3,
  ...
  return {
      output, output_state,      act_Wx,     act_Wx_norm,     act_Wx_norm_cache,
      act_Rh, act_Rh_norm_cache, act_c_norm, act_c_norm_cache};
}

std::vector<Tensor> layer_norm_lstm_backward(
    Tensor x_t, Tensor kernel_t, Tensor recurrent_kernel_t, Tensor bias,
    Tensor gamma, Tensor gamma_h, Tensor beta_h, Tensor zoneout_mask, Tensor h,
    Tensor c, Tensor act_Wx, Tensor act_Wx_norm, Tensor act_Wx_norm_cache,
    Tensor act_Rh, Tensor act_Rh_norm_cache, Tensor act_c_norm,
    Tensor act_c_norm_cache, Tensor dh_new, Tensor dc_new) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      x_t.scalar_type(), "layer_norm_lstm_backward", ([&] {
        ...
        layer_norm::BackwardPass<scalar_t> layer_norm1(
            time_steps * batch_size, hidden_size * 4, gamma_a[0].data(),
            nullptr, act_Wx.data_ptr<scalar_t>(), dgamma_a[0].data(), nullptr,
            act_Wx_norm_cache.data_ptr<scalar_t>());

        layer_norm::BackwardPass<scalar_t> layer_norm2(
            time_steps * batch_size, hidden_size * 4, gamma_a[1].data(),
            nullptr, act_Rh.data_ptr<scalar_t>(), dgamma_a[1].data(), nullptr,
            act_Rh_norm_cache.data_ptr<scalar_t>());

        layer_norm::BackwardPass<scalar_t> layer_norm3(
            time_steps * batch_size, hidden_size, gamma_h.data_ptr<scalar_t>(),
            beta_h.data_ptr<scalar_t>(), c_a[1].data(),
            dgamma_h.data_ptr<scalar_t>(), dbeta_h.data_ptr<scalar_t>(),
        ...
        lstm.Run(time_steps, kernel_t.data_ptr<scalar_t>(),
                 ...
                 act_Wx.data_ptr<scalar_t>(), layer_norm1,
                 act_Wx_norm.data_ptr<scalar_t>(), act_Rh.data_ptr<scalar_t>(),
                 layer_norm2, layer_norm3, act_c_norm.data_ptr<scalar_t>(),
                 has_zoneout ? zoneout_mask.data_ptr<scalar_t>() : nullptr);
      }));
...
void layer_norm_lstm_init(py::module &m) {
  m.def("layer_norm_lstm_forward", &layer_norm_lstm_forward,
        "LayerNormLSTM forward", py::call_guard<py::gil_scoped_release>());
  m.def("layer_norm_lstm_backward", &layer_norm_lstm_backward,
        "LayerNormLSTM backward", py::call_guard<py::gil_scoped_release>());
}
