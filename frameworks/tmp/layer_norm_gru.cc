...
std::vector<Tensor> layer_norm_gru_forward(bool training, float zoneout_prob,
                                           Tensor x, Tensor h0, Tensor kernel,
                                           Tensor recurrent_kernel, Tensor bias,
                                           Tensor recurrent_bias, Tensor gamma,
                                           Tensor zoneout_mask) {
  ...
  Tensor act_Wx =
      torch::empty({time_steps, batch_size, hidden_size * 3}, options);
  Tensor tmp_Wx_norm =
      torch::empty({time_steps, batch_size, hidden_size * 3}, options);
  ...
  AT_DISPATCH_FLOATING_TYPES(
      x.scalar_type(), "layer_norm_gru_forward", ([&] {
        ...
        gru.Run(time_steps, kernel.data_ptr<scalar_t>(),
                recurrent_kernel.data_ptr<scalar_t>(),
                bias.data_ptr<scalar_t>(), recurrent_bias.data_ptr<scalar_t>(),
                x.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(),
                cache.data_ptr<scalar_t>(), act_Wx.data_ptr<scalar_t>(),
                layer_norm1, tmp_Wx_norm.data_ptr<scalar_t>(),
                act_Rh.data_ptr<scalar_t>(), layer_norm2,
...
std::vector<Tensor>
layer_norm_gru_backward(Tensor x_t, Tensor kernel_t, Tensor recurrent_kernel_t,
                        Tensor bias, Tensor recurrent_bias, Tensor gamma,
                        Tensor zoneout_mask, Tensor h, Tensor cache,
                        Tensor act_Wx, Tensor act_Wx_norm_cache, Tensor act_Rh,
                        Tensor act_Rh_norm_cache, Tensor dh_new) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      x_t.scalar_type(), "layer_norm_gru_backward", ([&] {
        auto gamma_a = gamma.packed_accessor32<scalar_t, 2>();
        ...
        gru.Run(time_steps, kernel_t.data_ptr<scalar_t>(),
                recurrent_kernel_t.data_ptr<scalar_t>(),
                bias.data_ptr<scalar_t>(), recurrent_bias.data_ptr<scalar_t>(),
                x_t.data_ptr<scalar_t>(), h.data_ptr<scalar_t>(),
                cache.data_ptr<scalar_t>(), dh_new.data_ptr<scalar_t>(),
                dx.data_ptr<scalar_t>(), dW.data_ptr<scalar_t>(),
                dR.data_ptr<scalar_t>(), dbx.data_ptr<scalar_t>(),
                dbr.data_ptr<scalar_t>(), dh.data_ptr<scalar_t>(),
                dp.data_ptr<scalar_t>(), dq.data_ptr<scalar_t>(), layer_norm1,
                layer_norm2,
                has_zoneout ? zoneout_mask.data_ptr<scalar_t>() : nullptr);
      }));
...
void layer_norm_gru_init(py::module &m) {
  m.def("layer_norm_gru_forward", &layer_norm_gru_forward,
        "LayerNormGRU forward", py::call_guard<py::gil_scoped_release>());
  m.def("layer_norm_gru_backward", &layer_norm_gru_backward,
        "LayerNormGRU backward", py::call_guard<py::gil_scoped_release>());
}
