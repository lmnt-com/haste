...
std::vector<Tensor>
layer_norm_indrnn_forward(bool training, float zoneout_prob, Tensor x,
                          Tensor h0, Tensor kernel, Tensor recurrent_scale,
                          Tensor bias, Tensor gamma, Tensor zoneout_mask) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      x.scalar_type(), "layer_norm_indrnn_forward", ([&] {
        auto gamma_a = gamma.packed_accessor32<scalar_t, 2>();

        layer_norm::ForwardPass<scalar_t> layer_norm1(
            time_steps * batch_size, hidden_size, gamma_a[0].data(), nullptr,
        ...
        forward.Run(time_steps, kernel.data_ptr<scalar_t>(),
                    recurrent_scale.data_ptr<scalar_t>(),
                    bias.data_ptr<scalar_t>(), x.data_ptr<scalar_t>(),
                    output.data_ptr<scalar_t>(), workspace.data_ptr<scalar_t>(),
...
std::vector<Tensor>
layer_norm_indrnn_backward(Tensor x_t, Tensor kernel_t, Tensor recurrent_scale,
                           Tensor bias, Tensor gamma, Tensor zoneout_mask,
                           Tensor h, Tensor act_Wx, Tensor act_Wx_norm_cache,
                           Tensor dh_new) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      x_t.scalar_type(), "layer_norm_indrnn_backward", ([&] {
        ...
        backward.Run(time_steps, kernel_t.data_ptr<scalar_t>(),
                     ...
                     h.data_ptr<scalar_t>(), dh_new.data_ptr<scalar_t>(),
                     dx.data_ptr<scalar_t>(), dW.data_ptr<scalar_t>(),
                     du.data_ptr<scalar_t>(), db.data_ptr<scalar_t>(),
                     dh.data_ptr<scalar_t>(), workspace.data_ptr<scalar_t>(),
                     layer_norm1,
                     has_zoneout ? zoneout_mask.data_ptr<scalar_t>() : nullptr);
      }));
...
void layer_norm_indrnn_init(py::module &m) {
  m.def("layer_norm_indrnn_forward", &layer_norm_indrnn_forward,
        "LayerNormIndRNN forward", py::call_guard<py::gil_scoped_release>());
  m.def("layer_norm_indrnn_backward", &layer_norm_indrnn_backward,
        "LayerNormIndRNN backward", py::call_guard<py::gil_scoped_release>());
}
