...
std::vector<Tensor> ligru_forward(bool training, Tensor wx, Tensor h_init,
  ...
  Tensor act_uh =
      torch::zeros({seq_length, batch_size, hidden_size * 2}, options);
  ...
  AT_DISPATCH_FLOATING_TYPES(
      wx.scalar_type(), "ligru_layer_norm_forward", ([&] {
        ...
        forward.Run(seq_length, wx.data_ptr<scalar_t>(), u.data_ptr<scalar_t>(),
...
std::vector<Tensor> ligru_backward(Tensor wx_t, Tensor u_t, Tensor h,
  ...
  Tensor tmp_dwx =
      torch::zeros({time_steps, batch_size, hidden_size * 2}, options);
  ...
  AT_DISPATCH_FLOATING_TYPES(
      wx_t.scalar_type(), "ligru_2_0_backward", ([&] {
        auto gamma_a = gamma.packed_accessor32<scalar_t, 2>();
        auto dgamma_a = dgamma.packed_accessor32<scalar_t, 2>();

        layer_norm::BackwardPass<scalar_t> layer_norm1(
            time_steps * batch_size, hidden_size * 2, gamma_a[0].data(),
            nullptr, act_uh.data_ptr<scalar_t>(), dgamma_a[0].data(), nullptr,
        ...
        backward.Run(time_steps, wx_t.data_ptr<scalar_t>(),
                     u_t.data_ptr<scalar_t>(), h.data_ptr<scalar_t>(),
...
void ligru_2_0_init(py::module &m) {
  m.def("ligru_2_0_forward", &ligru_forward, "Li-GRU 2.0 forward",
        py::call_guard<py::gil_scoped_release>());
  m.def("ligru_2_0_backward", &ligru_backward, "Li-GRU 2.0 backward",
        py::call_guard<py::gil_scoped_release>());
}
