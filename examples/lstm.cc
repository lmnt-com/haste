...
void LstmInference(const Tensor2& W, const Tensor2& R, const Tensor1& b, const Tensor3& x) {
  ...
  for (int t = 0; t < time_steps; ++t) {
    ...
    float* v = v_dev.data + t * batch_size * hidden_size * 4;
...
void LstmTrain(const Tensor2& W, const Tensor2& R, const Tensor1& b, const Tensor3& x,
  ...
  device_ptr<Tensor3> h_dev(batch_size * hidden_size * time_steps);
  device_ptr<Tensor3> c_dev(batch_size * hidden_size * time_steps);
  ...
  {
    ScopeTimer t("Training forward pass time:");
    ForwardPass<float> forward = ForwardPass<float>(
        true,  // training
        batch_size,
        input_size,
    ...
    float* c = c_dev.data;
    for (int t = 0; t < time_steps; ++t) {
      const float* x_cur = x_dev.data + t * batch_size * input_size;
      float* h_out = h_dev.data + t * batch_size * hidden_size;
      float* c_out = c_dev.data + t * batch_size * hidden_size;
      float* v = v_dev.data + t * batch_size * hidden_size * 4;
  ...
  Tensor3 h(hidden_size, batch_size, time_steps);
  Tensor3 c(hidden_size, batch_size, time_steps);
  Tensor3 v(hidden_size * 4, batch_size, time_steps);
  h_dev.ToHost(h);
  c_dev.ToHost(c);
  v_dev.ToHost(v);

  Eigen::array<int, 3> transpose_3({ 1, 0, 2 });
  Tensor3 x_t = x.shuffle(transpose_3);
  Tensor3 h_t = h.shuffle(transpose_3);
  ...
  device_ptr<Tensor3> h_t_dev(h_t);
  device_ptr<Tensor2> zeros(batch_size * hidden_size);
  ...
  device_ptr<Tensor3> dv_dev(time_steps * batch_size * hidden_size * 4);
  ...
  {
    ScopeTimer t("Training backward pass time:");
    BackwardPass<float> backward = BackwardPass<float>(
        batch_size,
        input_size,
        hidden_size,
        g_blas_handle);
    ...
    for (int t = time_steps - 1; t >= 0; --t) {
      const float* x_t_cur = x_t_dev.data + t * batch_size * input_size;

      float* h_t = t != 0 ? h_t_dev.data + (t-1) * batch_size * hidden_size : zeros.data;
      float* c = t != 0 ? c_dev.data + (t-1) * batch_size * hidden_size : zeros.data;
      float* v = v_dev.data + t * batch_size * hidden_size * 4;

      float* c_new = c_dev.data + t * batch_size * hidden_size;
      float* dh_new = dh_new_dev.data + t * batch_size * hidden_size;
      float* dc_new = dc_new_dev.data + t * batch_size * hidden_size;
      float* dx = dx_dev.data + t * batch_size * input_size;
      float* dv = dv_dev.data + t * batch_size * hidden_size * 4;
      backward.Iterate(
          W_t_dev.data,
          R_t_dev.data,
          b_dev.data,
...
int main() {
  ...
  // Gradients.
  Tensor3 dh(HIDDEN_DIMS, BATCH_SIZE, SEQUENCE_LEN);
  Tensor3 dc(HIDDEN_DIMS, BATCH_SIZE, SEQUENCE_LEN);
