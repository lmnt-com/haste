...
__global__ void PointwiseOperations(
    const int batch_dim, const int hidden_dim, const T *c, const T *v,
    const T *c_new, const T *dh_new, const T *dc_new, T *db_out, T *dh_inout,
    T *dc_inout, T *dv_out,
...
template <typename T>
void BackwardPass<T>::Iterate(const cudaStream_t &stream,
                              const T *W_t,    // [H*4,C]
                              const T *R_t,    // [H*4,H]
                              const T *b,      // [H*4]
                              const T *x_t,    // [C,N]
  ...
  IterateInternal(R_t, c, c_new, dh_new, dc_new, db, dh, dc, v, zoneout_mask);
  ...
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_N, input_size, batch_size,
                hidden_size * 4, &alpha, W_t, input_size, v, hidden_size * 4,
  ...
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_T, hidden_size * 4,
                hidden_size, batch_size, &alpha, v, hidden_size * 4, h,
                hidden_size, &beta_sum, dR, hidden_size * 4);

  cublasSetStream(blas_handle, stream3);
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_N, hidden_size * 4,
                input_size, batch_size, &alpha, v, hidden_size * 4, x_t,
                batch_size, &beta_sum, dW, hidden_size * 4);
...
void BackwardPass<T>::IterateInternal(const T *R_t,    // [H*4,H]
  ...
  if (zoneout_mask) {
    PointwiseOperations<T, true><<<gridDim, blockDim, 0, stream1>>>(
        batch_size, hidden_size, c, v, c_new, dh_new, dc_new, db, dh, dc, v,
        zoneout_mask);
  } else {
  ...
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_N, hidden_size, batch_size,
                hidden_size * 4, &alpha, R_t, hidden_size, v, hidden_size * 4,
...
void BackwardPass<T>::Run(const int steps,
  ...
  for (int i = steps - 1; i >= 0; --i) {
    IterateInternal(R_t, c + i * NH, c + (i + 1) * NH, dh_new + (i + 1) * NH,
                    dc_new + (i + 1) * NH, db, dh, dc, v + i * NH * 4,
  ...
  cublasSetStream(blas_handle, stream2);
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_N, hidden_size * 4,
                input_size, batch_size * steps, &alpha, v, hidden_size * 4, x_t,
                batch_size * steps, &beta_sum, dW, hidden_size * 4);

  cudaStreamWaitEvent(stream3, event, 0);
  cublasSetStream(blas_handle, stream1);
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_T, hidden_size * 4,
                hidden_size, batch_size * steps, &alpha, v, hidden_size * 4, h,
                hidden_size, &beta_sum, dR, hidden_size * 4);

  cublasSetStream(blas_handle, stream1);
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_N, input_size,
                steps * batch_size, hidden_size * 4, &alpha, W_t, input_size, v,
                hidden_size * 4, &beta_assign, dx, input_size);
