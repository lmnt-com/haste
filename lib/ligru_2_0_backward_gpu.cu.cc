...
template <typename T>
__global__ void
PointwiseOperationsReLU(const int batch_dim, const int hidden_dim, const T *h,
...
template <typename T>
__global__ void
PointwiseOperationsLeakyReLU(const int batch_dim, const int hidden_dim, const T *h,
                    const T *v, T *dh_prev, const T *grad_out,
                    T *dwx) { // Zoneout mask (only used if ApplyZoneout==true)
  const int row = blockDim.x * blockIdx.x + threadIdx.x;
  const int col = blockDim.y * blockIdx.y + threadIdx.y;
...
template <typename T>
__global__ void
PointwiseOperationsTanh(const int batch_dim, const int hidden_dim, const T *h,
                    const T *v, T *dh_prev, const T *grad_out,
                    T *dwx) { // Zoneout mask (only used if ApplyZoneout==true)
  const int row = blockDim.x * blockIdx.x + threadIdx.x;
  const int col = blockDim.y * blockIdx.y + threadIdx.y;
...
template <typename T>
__global__ void
PointwiseOperationsSin(const int batch_dim, const int hidden_dim, const T *h,
                    const T *v, T *dh_prev, const T *grad_out,
                    T *dwx) { // Zoneout mask (only used if ApplyZoneout==true)
  const int row = blockDim.x * blockIdx.x + threadIdx.x;
  const int col = blockDim.y * blockIdx.y + threadIdx.y;
