...
__global__ void PointwiseOperations(
    ...
    const T *bx, const T *br, const T *h, T *h_out, T *v,
...
void ForwardPass<T>::IterateInternal(
    ...
    T *tmp_Wx_norm, // [N,H*3]
    T *act_Rh,      // [N,H*3]
    layer_norm::ForwardPass<T> &layer_norm2, T *tmp_Rh_norm,
    const float zoneout_prob,
  ...
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_N, hidden_size * 3,
                ...
                hidden_size, &beta, act_Rh, hidden_size * 3);
  ...
  if (training) {
    if (zoneout_prob && zoneout_mask) {
      PointwiseOperations<T, true, true><<<gridDim, blockDim, 0, stream1>>>(
          batch_size, hidden_size, tmp_Wx_norm, tmp_Rh_norm, bx, br, h, h_out,
          v, zoneout_prob, zoneout_mask);
    } else {
    ...
    if (zoneout_prob && zoneout_mask) {
      PointwiseOperations<T, false, true><<<gridDim, blockDim, 0, stream1>>>(
          batch_size, hidden_size, tmp_Wx_norm, tmp_Rh_norm, bx, br, h, h_out,
          nullptr, zoneout_prob, zoneout_mask);
    } else {
...
void ForwardPass<T>::Run(const int steps,
                         ...
                         T *tmp_Rh_norm, const float zoneout_prob,
                         const T *zoneout_mask) { // Zoneout mask [N,H]
  static const T alpha = static_cast<T>(1.0);
  static const T beta = static_cast<T>(0.0);
  ...
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_N, hidden_size * 3,
                ...
                input_size, &beta, act_Wx, hidden_size * 3);
  ...
  for (int i = 0; i < steps; ++i) {
    IterateInternal(R, bx, br, h + i * NH, h + (i + 1) * NH, v + i * NH * 4,
                    tmp_Wx_norm + i * NH * 3, act_Rh + i * NH * 3, layer_norm2,
                    tmp_Rh_norm, zoneout_prob,
