...
void ForwardPass<T>::Iterate(
  ...
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_N, hidden_size * 4,
                batch_size, input_size, &alpha, W, hidden_size * 4, x,
                input_size, &beta, v, hidden_size * 4);
  cudaEventRecord(event, stream2);

  IterateInternal(R, b, h, c, h_out, c_out, v, tmp_Rh, zoneout_prob,
                  zoneout_mask);
...
void ForwardPass<T>::IterateInternal(
  ...
  blas<T>::gemm(blas_handle, CUBLAS_OP_N, CUBLAS_OP_N, hidden_size * 4,
                ...
                hidden_size, &beta, tmp_Rh, hidden_size * 4);
  ...
  if (training) {
    if (zoneout_prob && zoneout_mask) {
      PointwiseOperations<T, true, true><<<gridDim, blockDim, 0, stream1>>>(
          batch_size, hidden_size, v, tmp_Rh, b, h, c, h_out, c_out, v,
          zoneout_prob, zoneout_mask);
    } else {
      PointwiseOperations<T, true, false><<<gridDim, blockDim, 0, stream1>>>(
          batch_size, hidden_size, v, tmp_Rh, b, h, c, h_out, c_out, v, 0.0f,
          nullptr);
    }
  } else {
    if (zoneout_prob && zoneout_mask) {
      PointwiseOperations<T, false, true><<<gridDim, blockDim, 0, stream1>>>(
          batch_size, hidden_size, v, tmp_Rh, b, h, c, h_out, c_out, nullptr,
          zoneout_prob, zoneout_mask);
    } else {
...
void ForwardPass<T>::Run(
  ...
  for (int i = 0; i < steps; ++i) {
    ...
    IterateInternal(R, b, h + i * NH, c + i * NH, h + (i + 1) * NH,
                    c + (i + 1) * NH, v + i * NH * 4, tmp_Rh, zoneout_prob,
