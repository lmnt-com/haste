...
class BackwardPass {
  public:
    ...
    // dc_new: [N,H] the gradient of `c_out` with respect to the loss at this iteration.
    // dx: [N,C] the gradient of the input at this time step with respect to the loss.
    // dW: [C,H*4] the gradient of the input weight matrix with respect to the loss.
    // dR: [H,H*4] the gradient of the recurrent weight matrix with respect to the loss.
    // db: [H*4] the gradient of the bias vector with respect to the loss.
    ...
    //     for the T-1'th iteration and the same pointer should be passed in for each
    //     iteration. After a complete backward pass, this vector will contain the gradient
    //     of the initial cell state with respect to the loss.
    // dv: [N,H*4] additional temporary work space required for this iteration. The caller
